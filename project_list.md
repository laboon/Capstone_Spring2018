...
_ONTAP Select Deploy_ - NetApp is developing a product called ONTAP Select Deploy which is a Debian-based VM for orchestrating the deployment of ONTAP Select clusters in a hypervisor environment. For Deploy, we are interested in the future of allowing SSO type login, especially for customers who are using ESX. VMware exposes APIs for authenticating against a vCenter. That is, they offer external identity verification like FB, Google, etc. do for the web.
